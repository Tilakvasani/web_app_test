...
async def chat_interaction(body: ChatRequest):
    ...
    streaming responses via SSE. Includes full conversation stream caching
    to serve identical repeat calls instantly for 0 tokens.

    NOTE: Token refresh is handled by a background task in web_app.py lifespan —
    it is NOT called here anymore to avoid blocking every request.
    """
    state = load_state()
